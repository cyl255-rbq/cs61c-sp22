...
long long int sum_simd_unrolled(int vals[NUM_ELEMS]) {
    ...
    for(unsigned int w = 0; w < OUTER_ITERATIONS; w++) {
        ...
        for(unsigned int i = 0; i < NUM_ELEMS / 16 * 16; i += 16) {
            __m128i tmp;
            __m128i mask;
            tmp = _mm_loadu_si128((__m128i *) (vals + i + 0));
            sum_vec0 = _mm_add_epi32(sum_vec0, _mm_and_si128(tmp, _mm_cmpgt_epi32(tmp, _127)));
            tmp = _mm_loadu_si128((__m128i *) (vals + i + 4));
            sum_vec1 = _mm_add_epi32(sum_vec1, _mm_and_si128(tmp, _mm_cmpgt_epi32(tmp, _127)));
            tmp = _mm_loadu_si128((__m128i *) (vals + i + 8));
            sum_vec2 = _mm_add_epi32(sum_vec2, _mm_and_si128(tmp, _mm_cmpgt_epi32(tmp, _127)));
            tmp = _mm_loadu_si128((__m128i *) (vals + i + 12));
            sum_vec3 = _mm_add_epi32(sum_vec3, _mm_and_si128(tmp, _mm_cmpgt_epi32(tmp, _127)));
        }
        ...
        for (unsigned int i = NUM_ELEMS / 16 * 16; i < NUM_ELEMS / 4 * 4; i += 4) {
            ...
            __m128i mask = _mm_cmpgt_epi32(tmp, _127);
            tmp = _mm_and_si128(tmp, mask);
            sum_vec0 = _mm_add_epi32(sum_vec0, tmp);
        }
        __m128i sum_vec = _mm_add_epi32(_mm_add_epi32(sum_vec0, sum_vec1), 
                               _mm_add_epi32(sum_vec2, sum_vec3));
        int tmp_arr[4];
        _mm_storeu_si128((__m128i *) tmp_arr, sum_vec);
        result += tmp_arr[0] + tmp_arr[1] + tmp_arr[2] + tmp_arr[3];
        for (unsigned int i = NUM_ELEMS / 4 * 4; i < NUM_ELEMS; i++)
